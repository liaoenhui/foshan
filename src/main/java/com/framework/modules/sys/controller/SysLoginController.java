...
//		}
		...
		try {
			...
			if(StringUtils.isBlank(decryptPWD)) decryptPWD = password;
	 ...
	 */
	@ResponseBody//method = RequestMethod.POST
	@RequestMapping(value ={ "/sys/loginRest"}, method = RequestMethod.POST)
	@ApiOperation("用户登录")
	@ApiImplicitParams(
	...
	public R loginRest(String username, String password, String captcha,HttpServletRequest req,HttpServletResponse resp) {
		R r = R.ok();
		String decryptPWD = DESUtil.decrypt(password);
		try {
			Subject subject = ShiroUtils.getSubject();
			SysUserEntity user = new SysUserEntity();
			user.setUsername(username);
