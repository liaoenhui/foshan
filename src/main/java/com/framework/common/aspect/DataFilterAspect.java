...
public class DataFilterAspect {
	...
	private String getSQLFilter(SysUserEntity user, JoinPoint point) {
		...
		String sqlFilterStart = " (";
		sqlFilter.append(sqlFilterStart);
		...
		if(dataFilter.domain()){
			sqlFilter.append(tableAlias).append(dataFilter.domainId()).append("=").append(user.getDomainId());
		}
		
		String sqlFilterEnd = ")";
		sqlFilter.append(sqlFilterEnd);
		if(sqlFilter.toString().equals(sqlFilterStart+sqlFilterEnd)){
			return "";
		}
